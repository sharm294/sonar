        ...
        for testVector in rawData["data"]:
            ...
            for parallelSection in testVector["data"]:
                ...
                for packet in parallelSection["data"]:
                    newData_parallelSection["data"], loopFound = expandingFunction(
                        packet, newData_parallelSection["data"], loopFound
                    )
...
def convertValue(packet):
    if "type" in packet:
        if "value" in packet:
            if not isinstance(packet["value"], (int)):
                if isinstance(packet["value"], (float)):
                    packet["value"] = int(packet["value"])
                else:
                    if packet["value"][:1] == "{":
    ...
    for testVector in rawData["data"]:
        for parallelSection in testVector["data"]:
            for packet in parallelSection["data"]:
                ...
                if packetType not in sonar_types and not packetType.startswith(
                    "signal"
                ):
                    currInterface = getInterface(packet["type"])
